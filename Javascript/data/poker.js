...
for (var i = 0; i < input.length; i++) {
	hands.push([input[i].slice(0, 14), input[i].slice(15)]);
}
...
Hand.prototype.best = function () {
  ...
  for (var i = 0; i < 4; i++) {
    ...
    flush = flush && (this.cards[i].suit === this.cards[i+1].suit);
  }
...
var count = 0;
for (var i = 0; i < 5; i++) {
  var result = compareHands(new Hand(hands[i][0]), new Hand(hands[i][1]));
  if (result === 1) count++;
}