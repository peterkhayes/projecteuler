var input = ['8C TS KC 9H 4S 7D 2S 5D 3S AC',
...
var Card = function(str) {
  this.suit = str.slice(str.length - 1);
...
var Hand = function (str) {
  ...
  this.freqs = freqs;
  this.cards = cards.sort(function (a, b) { return a.val - b.val; });
};

Hand.prototype.best = function () {
  // Test for straights and flushes.
  var straight = true;
  var flush = true;
  for (var i = 0; i < 4; i++) {
    straight = straight && (this.cards[i].val === this.cards[i+1].val - 1);
    flush = flush && (this.cards[i].suit === this.cards[i+1].suit || i === 4);
  }

  // Test for mega awesome hands.
  if (straight && flush) {
    if (this.cards[0].val === 10) return 10; // Royal flush.
    return 9; // Straight flush.
  }
  if (flush) return 6;
  if (straight) return 5;

  // Test for frequency hands.
  var three = false;
  var pairs = 0;
  for (var val in this.freqs) {
    if (this.freqs[val] === 4) return 8; // Four of a kind
    if (this.freqs[val] === 3) three = true;
    if (this.freqs[val] === 2) pairs++;
  }

  if (three && pairs) return 7; // Full house.
  if (three) return 4; // Three-of-a-kind.
  return 1+pairs; // Zero, one, or two pair.
};

var compareHands = function (h1, h2) {
  var type = h1.best();
  var compare = comparator(type, h2.best());
  if (compare) return compare; // If the types don't match, return the winner.
  if (type === 10) { // All royal flushes tie.
    return 0;
  } else if (type === 9) { // Straight flush.
    return comparator(h1.cards[4], h2.cards[4]);
  } else if (type === 8) { // Four of a kind.
    return comparator(h1.cards[2], h2.cards[2]);
  } else if (type === 7) { // Full house.
    return comparator(h1.cards[2], h2.cards[2]);
  } else if (type === 6) { // Flush.
    for (var i = 4; i >= 0; i--) {
      compare = comparator(h1.cards[i], h2.cards[i]);
    ...
    return comparator(h1.cards[2], h2.cards[2]); // Still impossible to have hands that tie.
  } else if (type === 3) { // Two pair.
    var p1 = [], p2 = [], s1, s2;
    for (var c1 in h1.freqs) {
      if (h1.freqs[c1] === 2) p1.push(c1);
      else s1 = c1;
    }
    for (var c2 in h2.freqs) {
      if (h2.freqs[c2] === 2) p2.push(c2);
      else s2 = c2;
    }
    p1.sort();
    p2.sort();
    compare = comparator(p1[1], p2[1]);
    if (compare) return compare;
    compare = comparator(p1[0], p2[0]);
    if (compare) return compare;
    return comparator(s1, s2);
  } else if (type === 2) { // Pair.
    var p1, p2, s1 = [], s2 = [];
    for (var c1 in h1.freqs) {
      if (h1.freqs[c1] === 2) p1 = c1;
      else s1.push(c1);
    }
    for (var c2 in h2.freqs) {
      if (h2.freqs[c2] === 2) p2 = c2;
      else s2.push(c2);
    }
    s1.sort();
    s2.sort();
    compare = comparator(p1, p2);
    if (compare) return compare;
    for (var i = 2; i >= 0; i--) {
      compare = comparator(s1[i], s2[i]);
      if (compare) return compare;
    }
    return 0;
  } else { // For hands with no combinations.
    for (var j = 4; j >= 0; j--) {
      compare = comparator(h1.cards[j], h2.cards[j]);
    if (compare) return compare;
    }
  }
  return 0;
...
var comparator = function (a, b) {
  if (typeof a === "object") a = a.val;
  if (typeof b === "object") b = b.val;
  if (a > b) return 1;
  if (b > a) return -1;
  return 0;
};
...
var generateCard = function() {
  var val = ["2", "3", "4", "5", "6", "7", "8", "9", "T", "J", "Q", "K", "A"][randInt(0, 13)];
  var suit = ["S", "C", "H", "D"][randInt(0,4)];
  return val + suit + " ";
};
...
for (var i = 0; i < 50000; i++) {
  var newHands = "";
  for (var j = 0; j < 10; j++) {
    newHands += generateCard();
  }
  newHands = newHands.slice(0, newHands.length - 1);
  input.push(newHands);
}
...
for (var i = 0; i < input.length; i++) {
  var h1 = new Hand(input[i].slice(0, 14));
  var h2 = new Hand(input[i].slice(15));
  var result = compareHands(h1, h2);
  //console.log(handNames[h1.best()] + (result === 1 ? " beats " : (result === -1 ? " loses to " : " ties ")) + handNames[h2.best()]);
  //console.log("---");
  if (result === 1) count++;
}
